...
const titles: Record<string, { title: string; sub?: string }> = {
  "/app": { title: "Dashboard", sub: "Visão geral da operação" },
  ...
  "/app/contacts": { title: "Contatos", sub: "Sua base de clientes" },
...
export function AppLayout() {
  ...
  const meta = titles[pathname] ?? { title: "Whatomate" };
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="h-14 flex items-center gap-3 px-4 border-b border-border bg-background/80 backdrop-blur sticky top-0 z-30">
            ...
            <div className="ml-auto flex items-center gap-2">
              <Button variant="ghost" size="sm" className="gap-2 text-muted-foreground hover:text-foreground h-8 px-3 border border-border bg-secondary/40 rounded-md">
